...
async def getMessagesID(client):
    try:
        async for message in client.iter_messages('ydhsbsbad'):
            print(f'Message from {message.sender_id}: {message.text} : link https://t.me/ydhsbsbad/{message.id}')
    ...
    if client is not None:
        ...
        while True:
            ...
            if lastMessage == None or lastMessage != msg_id:
                ...
                task2 = asyncio.create_task(view_main(chat_id,msg_id))
                lastMessage = msg_id
